      ...
      >
        ...
        <el-row :gutter="20">
          <template v-if="item.origin">
            <!-- {{ item.origin }} -->
            <pre-view :origin="item.origin"></pre-view>
          </template>
          <template v-else>
            <div>{{ item.fileName }}</div>
          </template>
...
const sourceMapUpload = async (file: any) => {
  ...
  reader.onload = async function (event) {
    const code = await getSource(event.target?.result, stackFrameObj.line, stackFrameObj.column)
...
const getSource = async (sourcemap: any, line: number, column: number) => {
  try {
    ...
    ElMessage.error('sourceMap 解析失败')
  }
}
</script>
